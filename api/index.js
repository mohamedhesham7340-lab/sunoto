...
let memoryProducts = [];
let memorySettings = {
  storeName: "الفرعون الذهبي للتحف والهدايا الملكية",
  storeNameEn: "Golden Pharaoh Luxury Artifacts",
  tagline: "صناع التاريخ ومصممو الفخامة المصرية الأصيلة",
  phone: "+20 100 123 4567",
  whatsappNumber: "+201001234567",
  email: "sales@pharaoh-luxury.com",
  address: "خان الخليلي - شارع المعز، القاهرة التاريخية، مصر",
  currency: "EGP",
  usdRate: 0.021,
  sarRate: 0.078,
  aedRate: 0.076,
  ...
  minOrderWholesale: 5,
  headerNotice: "✨ شحن دولي متاح لجميع دول العالم مع شهادة أصالة معتمدة وتغليف ملكي فاخر",
...
if (dbConnectionString) {
  try {
    ...
    console.error('Failed to initialize PostgreSQL pool in Vercel API:', err);
  }
}
...
app.get('/api/db-status', async (req, res) => {
  if (pool) {
    try {
      ...
      try {
        ...
        return res.json({
          ...
          productCount: parseInt(prodCountRes.rows[0]?.count || '0', 10),
          quoteCount: parseInt(quoteCountRes.rows[0]?.count || '0', 10),
        });
      } finally {
        client.release();
      }
      ...
      return res.json({
        connected: false,
        type: 'memory',
        message: `تعذر الاتصال بـ Postgres: ${err?.message || 'خطأ'}.`,
    ...
    return res.json({
      connected: false,
      type: 'local_storage',
      message: 'لتوصيل Neon على Vercel، أضف DATABASE_URL في Environment Variables.',
      productCount: memoryProducts.length
    });
  }
});
...
app.post('/api/test-db-connection', async (req, res) => {
  ...
  try {
    ...
    return res.json({ success: true, message: 'تم الاتصال بقاعدة بيانات Neon بنجاح!' });
  } catch (err) {
    return res.status(400).json({ success: false, message: `فشل الاتصال: ${err?.message || 'خطأ'}` });
  }
});
...
app.get('/api/settings', async (req, res) => {
  ...
  return res.json({ ...memorySettings, databaseConnected: !!pool });
});
